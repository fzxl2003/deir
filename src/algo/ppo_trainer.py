...
class PPOTrainer(PPORollout):
    ...
    def __init__(
        ...
        int_rew_lb: float = -100,
        int_rew_ub: float = 100,
        enable_plotting: int = 0,
        can_see_walls: int = 1,
        atari_clip_rew: int = 0,
        reset_on_life_loss: int = 0,
        life_loss_penalty: float = 0.0,
        atari_img_size: int = 84,
        ...
        super(PPOTrainer, self).__init__(
            ...
            int_rew_lb=int_rew_lb,
            int_rew_ub=int_rew_ub,
            adv_momentum=adv_momentum,
            image_noise_scale=image_noise_scale,
            enable_plotting=enable_plotting,
            can_see_walls=can_see_walls,
            atari_img_size=atari_img_size,
            atari_clip_rew=atari_clip_rew,
            ...
            repeat_act_prob=repeat_act_prob,
        ...
        for epoch in range(max(self.n_epochs, self.model_n_epochs)):
            for rollout_data in self.ppo_rollout_buffer.get(self.batch_size):
                ...
                if epoch < self.n_epochs:
                    ...
                    values, values_ir, log_prob, entropy, memories = \
                    ...
                    if self.clip_range_vf is None:
                        ...
                        values_ir_pred = rollout_data.old_values_ir + th.clamp(
                            values_ir - rollout_data.old_values_ir, -clip_range_vf, clip_range_vf
                        )
                    # Value loss using the TD(gae_lambda) target
                    value_loss = F.mse_loss(rollout_data.returns, values_pred)
                    if self.int_rew_gamma > 0:
                        value_er_loss = value_loss
                        value_ir_loss = F.mse_loss(rollout_data.returns_ir, values_ir_pred)
                        value_loss = value_loss + value_ir_loss
                    else:
                        value_er_loss = None
                        value_ir_loss = None
                    ...
                    self.training_stats.add(
                        ...
                        value_er_loss=value_er_loss,
                        value_ir_loss=value_ir_loss,
