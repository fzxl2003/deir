...
from src.utils.common_func import get_norm_layer_1d
...
class PolicyValueOutputHeads(nn.Module):
    def __init__(
        ...
        p_modules = [
            ...
            get_norm_layer_1d(mlp_norm, self.latent_dim_pi),
            activation_fn(),
        ]
        for l in range(1, mlp_layers):
            p_modules += [
        ...
        v_modules = [
            ...
            get_norm_layer_1d(mlp_norm, self.latent_dim_vf),
            activation_fn(),
        ]
        for l in range(1, mlp_layers):
            v_modules += [
...
class ForwardModelOutputHeads(nn.Module):
    def __init__(
        ...
        modules = [
            ...
            get_norm_layer_1d(mlp_norm, latent_dim),
            activation_fn(),
        ]
        for l in range(1, mlp_layers):
            modules += [
...
class InverseModelOutputHeads(nn.Module):
    def __init__(
        ...
        modules = [
            ...
            get_norm_layer_1d(mlp_norm, latents_dim),
            activation_fn(),
        ]
        for l in range(1, mlp_layers):
            modules += [
...
class RNDOutputHeads(nn.Module):
    def __init__(self,
        ...
        # Stop using BatchNorm or LayerNorm in RND
        # to alleviate the issue of vanishing intrinsic rewards
...
class DiscriminatorOutputHeads(nn.Module):
    def __init__(
        ...
        self.use_rnd = use_rnd
        if use_rnd:
            self.rnd_model = RNDOutputHeads(
                latents_dim, latents_dim, latents_dim, activation_fn,
                mlp_norm, mlp_layers
            )
    ...
    def rnd_forward(self, curr_emb: Tensor) -> Tuple[Optional[Tensor], Optional[Tensor]]:
        if self.use_rnd:
            return self.rnd_model(curr_emb)
        return None, None
