...
class PPOModel(ActorCriticCnnPolicy):
        ...
        if self.int_rew_source in [ModelType.DEIR, ModelType.PlainDiscriminator]:
            self.int_rew_model = DiscriminatorModel(
                **int_rew_model_kwargs,
                ...
                dsc_use_rnd=self.dsc_use_rnd,
                dsc_l1_reg_coef=self.dsc_l1_reg_coef,
                dsc_l2_reg_coef=self.dsc_l2_reg_coef,
                dsc_clip_value=self.dsc_clip_value,
                dsc_ir_bias=self.dsc_ir_bias,
                dsc_tolerance=self.dsc_tolerance,
                dsc_ll_rew=self.dsc_ll_rew,
                dsc_visible_ratio=self.dsc_visible_ratio,
                dsc_header_mask=self.dsc_header_mask,
    ...
    def _init_modules(self) -> None:
        ...
        if self.int_rew_gamma > 0:
            self.value_net_ir = nn.Linear(self.latents_dim, 1)
            nn.init.zeros_(self.value_net_ir.weight)
            nn.init.zeros_(self.value_net_ir.bias)
    ...
    def forward(self, obs: Tensor, mem: Tensor, deterministic: bool = False):
        ...
        if self.int_rew_gamma > 0:
            values_ir = self.value_net_ir(latent_vf)
        else:
            values_ir = th.zeros_like(values)
        # distribution = self._get_action_dist_from_latent(latent_pi, latent_sde=latent_sde)
        mean_actions = self.action_net(latent_pi)
        if self.noop_bias != 0.0:
            mean_actions[:, 0] += self.noop_bias
        distribution = self.action_dist.proba_distribution(action_logits=mean_actions)

        actions = distribution.get_actions(deterministic=deterministic)
        log_prob = distribution.log_prob(actions)
        return actions, values, values_ir, log_prob, memories
    ...
    def evaluate_policy(self, obs: Tensor, act: Tensor, mem: Tensor):
        ...
        if self.int_rew_gamma > 0:
            ...
            values_ir = th.zeros_like(values)
        return values, values_ir, log_prob, distribution.entropy(), memories
