...
def test(config):
    ...
    #load
    model.load(config.load_model_path,run_id=config.run_id)
...
@click.option('--load_model_path', default="/root/project/deir/model.zip", type=str, help='Path to load the model')
def main(
    ...
    rec_interval, video_length, log_dsc_verbose, env_render, use_status_predictor, load_model_path
):
